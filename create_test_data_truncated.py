...
df_bhc["text"] = df_bhc.apply(
    lambda row: alpaca_prompt.format(
        ...
        outputs=row['brief_hospital_course']   ####################################### row['brief_hospital_course'] for unstructure , output will be '' for inference
    ),
    axis=1
)
...
df_ds["text"] = df_ds.apply(
    lambda row: alpaca_prompt.format(
        ...
        outputs=row['discharge_instructions']     ############################################### row['discharge_instructions'] for unstructure  , output will be '' for inference
    ),
    axis=1
)
...
df['bert_token_count_bhc'] = df['discharge_instructions'].apply(count_bert_tokens)
df['bert_token_count_di'] = df['brief_hospital_course'].apply(count_bert_tokens)
df = df[df['bert_token_count_text'] <= 8000].drop(columns=['bert_token_count_text'])
df = df[(df['bert_token_count_di'] <= 1000) & (df['bert_token_count_di'] > 100)].drop(columns=['bert_token_count_di'])
df = df[(df['bert_token_count_bhc'] <= 1000) & (df['bert_token_count_bhc'] > 100)].drop(columns=['bert_token_count_bhc'])

#### fine tune code which take the data we have created using create_test_data_full.py (generate train valid, test data)
...
tracker = EmissionsTracker(
            project_name="fine-tune-full-prompt",   ### for carbon emission track
...
df = pd.read_csv(PATH+"all_merged_data_train_truncated.csv") # converted train file (full)
dataset = Dataset.from_pandas(df)


df = pd.read_csv(PATH+"all_merged_data_valid_truncated.csv") # converted valid file (full)
eval_dataset = Dataset.from_pandas(df)


df = pd.read_csv(PATH+"all_merged_data_test_phase_2_truncated.csv") # converted test file (full)
...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        ...
        report_to = "none", #"wandb", #"none", # Use this for WandB etc
    ),
)
...
eval_results = trainer.evaluate() # if you do not want eval skip this, it takes lots of time




model.save_pretrained("./out_model/phi4/lora_model") # Local saving of trained model
tokenizer.save_pretrained("./out_model/phi4/lora_model")
...
def summerize(inp):
    ...
    out = tokenizer.batch_decode(outputs)[0].replace(inp, "")
    if '### Instruction:' in out and '### Response:' in out:
        # remove from "### Response:" onwards (including it)
        out = out[out.index("### Response:")+len('### Response:'):].strip()
...
for i in range(len(df)):
    ...
    if i%100==0:
        df['prediction'] = summ
        df.to_excel("test2_unsloth_phi4_as_it_is.xlsx", index = False) ## output file (generated result of test set)
